...
## 2026-09-23 补充核验（不改写原检索记录）

原记录基于可检索文本，未覆盖SI中的栅格化表格。本轮逐页渲染244-SI.pdf，
S4明确给出C60专用NN（62019构型）、合金NN（37223构型）、水团簇NN（53000构型），
S14给出C60平均509.9次能量评估/SSW步。不能再把纯文本未提取到解释为PDF没有相关信息；
仍未据此取得这些势的权重或各体系完整运行协议。详见[预算尺度核验](2026-09-23-c60-budget-scale.md)。
