...
状态：用户已批准最小方案；按后续整体优先级审视要求暂缓，未实施公共API或持久化格式变更。
...
本轮不依赖此设计的对照、读出、文献核验、驱动修复及归档均已完成。按照用户当前
AGENTS第8节，状态/持久化格式重设计与公共接口变更需先讨论；此前批准的是固定胞
SSW/LS/池状态恢复，尚未覆盖GA进行中的walk归属。
