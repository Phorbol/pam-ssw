...
**状态：`PROPOSED_NOT_SUBMITTED`。** 本文提出下一阶段候选协议，不构成作业提交、预算批准或成笼保证。是否排期应结合当前C4H6曲率资格读出，由主线负责人决定；如选择执行，再另行确认最多176 V100 GPU小时的预算。
...
| 设置 | 建议值 | 来源与边界 |
|---|---:|---|
| 新输入seed | 17101、17102 | 仅为提名；使用既有 `generate.py` 的均匀立方体拒绝采样入半径5 Å球、逐原子最小间距1 Å、平移至(25,25,25) Å、非周期C60规则。记录试采次数、原始坐标和清单；该输入分布是开发协议约定，不是文献最优分布。生成结构须在独立的授权准备阶段完成，本任务不生成。 |
...
**有两种可选路线：** A）C4H6资格读出后执行四轨迹C60比较，预算上限176 GPU小时，取得MH-1下长预算双验收和LS方向的有限证据；B）继续功能开发，C60双验收保持未完成，把这笔GPU预算保留给后续有更强决策信号的实验。当前推荐先完成C4H6资格读出，再由主线负责人根据其结果和机会成本选择A或B；现在不预设暂缓结论，也不代表预算已获批准。即使选择A，两个新初态仍不足以给出稳健成功率估计。
