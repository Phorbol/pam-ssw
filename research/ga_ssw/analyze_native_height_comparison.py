...
def main():
 ...
 expected=[(system,seed,arm) for system in ('c4h6','cu13') for seed in (3,17) for arm in ('forward_force','native_conservative')]
 actual={(r['system'],r['seed'],r['arm']):r for r in summary['runs']}
 if len(actual)!=8 or any(k not in actual or actual[k]['status']=='running' for k in expected):raise RuntimeError('all eight terminal runs required; never publish partial denominator')
 ...
 graphs=[];rows=[]
 for key in expected:
  raw=actual[key];name=f'{key[0]}-{key[2]}-seed{key[1]}';directory=BASE/name
