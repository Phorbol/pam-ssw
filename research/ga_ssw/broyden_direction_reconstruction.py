"""Research-only ASE-facing direction runner built from the recovered raw state.

Design boundary: this runner follows a *root-selection* direction problem.  It
returns a local stationary direction reachable from the supplied anchor within
the bounded Broyden iteration, and makes no lowest-eigenmode claim.
...
No production module is imported for behavior changes and no ASE calculator is
run unless the caller invokes :func:`broyden_direction`.
"""
...
from .broyden_state_reconstruction import BroydenState
...
def broyden_direction(
    ...
    *,
    ...
    while hvp_calls < max_hvp:
        ...
        if residual <= tol:
            return BroydenDirectionResult(direction.reshape(shape), curvature, residual,
                                          hvp_calls, force_calls, True, tuple(trace))
    ...
    return BroydenDirectionResult(direction.reshape(shape), curvature, residual,
                                  hvp_calls, force_calls, False, tuple(trace))
