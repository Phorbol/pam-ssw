...
def main():
 plan={'status':'prepared','pes_calls':0,'source':str(FROZEN),'directions':['atomic_translation_projected','strain6','mixed_projected'],'point':'saved_stage8_failed_q; exact match required','steps':[1e-4,5e-5],'expected_requests':13,'cap':15,'wall_seconds':90,'seed':20260911,'purpose':'analytic directional derivative vs central FD at saved failed endpoint; not physical success'}
 (OUT/'failed-endpoint-fd-plan.json').write_text(json.dumps(plan,indent=2)+'\n')
 ...
 center=np.asarray(frozen['center_q'],float); saved=np.asarray(frozen['saved_stage8_failed_q'],float); assert saved.shape==(150,); assert np.array_equal(saved,np.asarray(json.loads((BASE/'joint-stage8-diagnostic-v1/result.json').read_text())['saved_stage8']['q'],float)); terms=[(np.asarray(t['center']),np.asarray(t['direction']),float(t['weight'])) for t in frozen['previous_terms']]; q0=saved
 ...
 out={'status':'complete','requests':surface.requests,'wall_seconds':time.monotonic()-plan_start,'source_sha256':sha(SOURCE),'frozen_sha256':sha(FROZEN),'model_sha256':sha(MODEL),'environment':{'python':str(Path(__import__('sys').executable).resolve()),'numpy':numpy.__version__,'ase':ase.__version__,'torch':torch.__version__,'mace':mace.__version__,'ase_file':ase.__file__,'numpy_file':numpy.__file__},'point':'saved_stage8_failed_q (exact diagnostic saved_stage8.q)', 'center':{'q':q0.tolist(),'objective':e0,'analytic_gradient':g0.tolist()},'directions':rows,'ledger':ledger,'note':'directional derivative check only; no optimization or physical qualification'}
 (OUT/'failed-endpoint-fd-result.json').write_text(json.dumps(out,indent=2,allow_nan=False)+'\n')
