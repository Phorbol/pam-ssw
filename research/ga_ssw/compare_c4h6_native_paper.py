...
OUT=Path('research/ga_ssw/evidence/c4h6-native-paper-two-step')
...
def main():
    ...
    dump(out/'plan.json',dict(seeds=[3,17],arms=['ssw','paper','native'],steps=2,
        per_arm_total_EF_cap=2000,total_cap=12000,total_wall_seconds=600,threads=1,
        ...
        for seed in (3,17):
            for arm in ('ssw','paper','native'):
                ...
                class Counted(ASESurface):
                    ...
                    def evaluate(self,a):
                        if used[0]>=2000 or time.monotonic()>=deadline:raise Budget('per-arm request/shared wall cap')
                        used[0]+=1
                ...
                try:
                    ...
                    if time.monotonic()>=deadline or used[0]>=2000:row['budget_reached']=True
