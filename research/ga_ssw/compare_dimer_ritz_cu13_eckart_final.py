...
def main():
    out=Path('research/ga_ssw/evidence/dimer-ritz-cu13-eckart-final')
    out.mkdir(parents=True,exist_ok=False)
    (out/'run_script.py').write_text(Path(__file__).read_text())
    source=out/'source';source.mkdir()
    for filename in ['paper_reference.py','cluster_frame.py','surface.py','direction.py','dimer.py','gaussian.py']:
        (source/filename).write_text((Path('pamssw/standalone')/filename).read_text())
    atoms=read('research/ga_ssw/evidence/independent-cu13-surface/quenched.extxyz')
    write(out/'initial.extxyz',atoms)
    base=dict(cluster_frame="eckart",width=.2,rotation_bias=100.,max_gaussians=14,temperature_K=300.,
    ...
    plan=dict(system='Cu13',backend='ASE EMT',seeds=[3,17],steps=15,config=base,
              ...
              purpose='paired coordinate-section intervention; same saved input/seeds/budgets as Cartesian failure; not efficacy campaign',
    ...
    for seed in plan['seeds']:
        for solver in plan['solvers']:
            ...
            row=dict(seed=seed,solver=solver,status=result.status,stages=[r.status for r in result.records],
                     requests=surface.requests,landings=len(result.minima),fresh_checks=checks,energy_span=float(max(c['energy'] for c in checks)-min(c['energy'] for c in checks)),wall_seconds=time.monotonic()-started)
