...
OUT=Path('research/ga_ssw/evidence/hard-c60-gfn2-one-step-2000')
...
def main():
    ...
    atoms=read('research/ga_ssw/evidence/ccd-c60-gfn2-qualification/ccd1809asym-after.extxyz');write(out/'input.extxyz',atoms)
    ...
    dump(out/'plan.json',dict(seeds=[3],arms=['ssw','paper'],steps=1,
        per_arm_total_EF_cap=2000,search_cap=1998,fresh_reserve=2,total_cap=4000,per_arm_wall_seconds=900,total_wall_seconds=1800,threads=1,
        config=config,paper_ls=paper_ls,
        backend='tblite 0.7 GFN2-xTB accuracy .001; CPU',
        development_costs=dict(qualification_EF=39,earlier_Ih_EF=1292),source='CCD1809asym GFN2-qualified non-Ih cage, no Hessian;  LS paper SI section3 target .02 eV/atom; SI7.3/7.4 T150 width.6 NG12; common stricter fmax.01 Safe400/dimerHVP100/fd1e-4/tol.02 from C4H6 numerical baseline',
        ...
        failure='no retry/retune; preserve incomplete logs; independent per-arm wall guard includes independent checks'))
    ...
    def alarm(*_):raise Budget('independent arm 900-second wall deadline')
    ...
    baseline,_=graph(atoms,cutoffs);ihgraph,_=graph(molecule('C60'),cutoffs);runs=[]
    try:
        for seed in (3,):
            for arm in ('ssw','paper'):
                deadline=time.monotonic()+900;signal.setitimer(signal.ITIMER_REAL,900)
                ...
                class Counted(ASESurface):
                    ...
                    def evaluate(self,a):
                        if used[0]>=(2000 if self.fresh else 1998) or time.monotonic()>=deadline:
                            row['budget_reached']=True
                            raise Budget('per-arm request/independent wall cap')
                        used[0]+=1
                ...
                try:
                    ...
                    r=paper.run_ssw(atoms,surface,steps=1,config=config,rng=np.random.default_rng(seed),ls=ls)
                    ...
                    for i,m in enumerate(r.minima):
                        ...
                        row['checks'].append(dict(index=i,energy=e,energy_error=e-m.energy,
                            ...
                            same_graph_as_Ih=nx.is_isomorphic(g,ihgraph,node_match=lambda x,y:x['Z']==y['Z']),carbon_degrees=sorted(dict(g.degree()).values()),radius_min=float(np.linalg.norm(m.atoms.positions-m.atoms.positions.mean(axis=0),axis=1).min()),radius_max=float(np.linalg.norm(m.atoms.positions-m.atoms.positions.mean(axis=0),axis=1).max()),radius_gyration=float(np.sqrt(np.mean(np.sum((m.atoms.positions-m.atoms.positions.mean(axis=0))**2,axis=1)))),edges=list(g.edges),atoms=m.atoms,distances=distances))
        ...
        dump(out/'summary.json',dict(runs=runs,total_requests=sum(x['total_requests'] for x in runs),wall_seconds=time.monotonic()-started,
            limits='two arms, one seed, one attempt; force/network/cage checks not Hessian or C60 GM proof; GFN2 not original NN/DFT; unequal completed work cannot imply efficiency gain'))
