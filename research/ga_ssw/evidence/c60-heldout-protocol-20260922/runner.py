...
from pamssw.standalone import (  # noqa: E402
    NativeMCSettings, RecoveredRotationSettings, SSWConfig,
...
def run_case(plan, arm, case):
    seed = plan["seeds"][case]
    folder = HERE / arm / f"{case}-seed{seed}"
    if folder.exists():
        raise FileExistsError(folder)
    folder.mkdir(parents=True)
    arm_config = plan["arms"][arm]
    ssw_config = dict(plan["common_ssw_config"])
    ssw_config["rotation_solver"] = arm_config["rotation_solver"]
    config = SSWConfig(**ssw_config)
    rotation = (None if arm_config["recovered_rotation"] is None else
                RecoveredRotationSettings(**arm_config["recovered_rotation"]))
    atoms = read(HERE / "inputs" / f"{case}.traj")
    write(folder / "input.traj", atoms)
    (folder / "effective-config.json").write_text(json.dumps({
        "arm": arm, "case": case, "seed": seed, "ssw_config": ssw_config,
        "recovered_rotation": arm_config["recovered_rotation"],
    ...
    row = {"arm": arm, "case": case, "seed": seed, "status": "started",
           "runtime_versions": runtime_versions()}
    ...
    try:
        ...
        mc=NativeMCSettings(plan["native_mc"]["energy_tol_eV"],
            ...
            checkpoint_path=checkpoint, recovered_rotation=rotation)
    ...
    assert row["search_requests"] <= plan["search_cap_per_arm"]
    assert row["fresh_initial_best_requests"] <= plan["fresh_per_arm"]
    assert row["fresh_conditional_requests"] <= plan["conditional_first_cage_extra_per_arm"]
    assert row["fresh_requests"] <= plan["fresh_per_arm"] + plan["conditional_first_cage_extra_per_arm"]
...
def main():
    ...
    parser.add_argument("--arm", choices=("baseline_broyden", "recovered_rotation"))
    parser.add_argument("--case", choices=("c60_17095", "c60_17096"))
    args = parser.parse_args()
    if not args.execute:
        print("prepared_not_executed")
        return
    if args.arm is None or args.case is None:
        parser.error("--arm and --case are required; each job owns one arm/case directory")
    plan = json.loads((HERE / "plan.json").read_text())
    check_frozen(plan)
    run_case(plan, args.arm, args.case)
