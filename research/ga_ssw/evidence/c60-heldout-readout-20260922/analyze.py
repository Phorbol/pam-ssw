...
ROOT = HERE.parent / 'c60-rotation-heldout-20260921'
...
def analyze(root):
    ...
    for case in plan['cases']:
        for name in ('baseline_broyden', 'recovered_rotation'):
            folder = root / name / f"{case}-seed{plan['seeds'][case]}"
            ...
            try:
                row = prior.arm(root / name, case, plan['seeds'][case], True, graph, reference_graph)
                row['arm'] = name
                ...
                if summary['search_requests'] > plan['search_cap_per_arm']:
                    errors.append(f'{case}/{name}: search budget exceeded')
                if summary['fresh_requests'] > plan['fresh_per_arm'] + plan['conditional_first_cage_extra_per_arm']:
    ...
    return dict(protocol='held-out two-input comparison; not a universal success-rate estimate',
