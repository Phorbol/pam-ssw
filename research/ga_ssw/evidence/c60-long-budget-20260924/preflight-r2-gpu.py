...
# CPU contract checks are a short setup phase of this genuinely GPU experiment.
# CPU-MISC was fully allocated; this uses the same already authorized GPU job.
repo=ROOT.parents[3]
with (ROOT/'preflight-r2/cpu-in-gpu.out').open('w') as out, (ROOT/'preflight-r2/cpu-in-gpu.err').open('w') as err:
    subprocess.run([sys.executable,'-m','pytest','-q',str(repo/'tests/standalone/test_c60_long_budget.py')],
                   cwd=repo,stdout=out,stderr=err,check=True)
for arm in ('ssw','native-ls'):
    base=ROOT/'preflight-r2'/arm
    def run(name,n):
        subprocess.run([sys.executable,str(ROOT/'preflight-r2-runner.py'),'--run-dir',str(base/name),
...
for arm in ('ssw','native-ls'):
    ...
    rows.append(row)
(ROOT/'preflight-r2/results.json').write_text(json.dumps(dict(rows=rows,elapsed_seconds=time.monotonic()-start),indent=2)+'\n')
