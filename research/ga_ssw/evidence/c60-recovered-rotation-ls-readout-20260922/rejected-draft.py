...
def ls_events(result):
    """Audit only fields actually emitted by the frozen result schema."""
    records=result.get("records", [])
    responses=[]; updates=[]; preparations=[]; failures=[]
    for record in records:
        if record.get("energy_response") is not None:
            responses.append(record["energy_response"])
        if record.get("ls_update") is not None:
            updates.append({"value": record["ls_update"],
                            "native_ls_update": record.get("native_ls_update")})
        if record.get("ls_preparation") is not None:
            preparations.append(record["ls_preparation"])
        if record.get("status") == "ls_update_failed" or record.get("ls_update_error") is not None:
            failures.append({"status": record.get("status"),
                             "error": record.get("ls_update_error"),
                             "record": record.get("index")})
    return {"records": len(records), "energy_response_eV_per_atom": responses,
            "energy_response_unit": "eV/atom (source ls_cycle.py contract)",
            "ls_update_count": len(updates), "ls_updates": updates,
            "ls_preparation_count": len(preparations),
            "ls_update_failures": failures,
            "failure_exit_statuses": sorted({r.get("status") for r in records
                                               if r.get("status") in {"ls_update_failed", "exception"}})}

def arm(root, case, seed, new, graph, reference_graph):
    folder=root/f"{case}-seed{seed}"
    ...
    if new:
        row["result"]["cost_reconciliation"]={
            "initial_plus_records_requests":sum([result["initial"].get("evaluation_requests", 0)] +
                                                  [r.get("evaluation_requests", 0) for r in result.get("records", [])]),
            "result_evaluation_requests":result.get("evaluation_requests"),
            "ledger_search_rows":row["ledger"].get("search_rows"),
            "ledger_search_failures":row["ledger"].get("search_failures"),
        }
        row["result"]["ls_audit"]=ls_events(result)
    return row
...
def main():
    ...
    a.output.write_text(json.dumps({"scope":"offline C60 validator readout; no PES and no automatic success claim","reference_energy_eV":EREF,"energy_tolerance_eV":ETOL,"graph_cutoffs_A":[1.8,1.64,1.7],"reference_path":str(a.reference),"new_cost_definition":"initial result request count plus every record evaluation_requests; fresh checks reported separately","old_comparison":"direct reuse of c60-recovered-rotation-long-20260921 result and ledger; absent LS fields are not errors","pairs":rows},indent=2)+"\n")
