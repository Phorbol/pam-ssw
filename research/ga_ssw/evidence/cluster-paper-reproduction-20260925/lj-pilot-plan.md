...
Post-hoc geometry rule (fixed before execution): use first-neighbor adjacency at1.3sigma, enumerate up to1000 isomorphisms and minimize proper-rotation RMS. Positive identity requires RMS<0.03sigma in addition to energy/force qualification. This loose geometry tolerance avoids chasing coordinate precision; a capped enumeration without a match is inconclusive. Targets are compared only after search.

Height is the existing adaptive forward-force rule, F_target=0.1eV/Angstrom, derived from BP-CBD2012 pp2217–2218 as cited in SSW2013 §2; it is not a fixed Gaussian height. Morse-specific reduced-unit realization remains an audit boundary.
