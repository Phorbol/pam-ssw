...
CPU1488209 exit0 in5s: known LJ55 reference stops at initial quench (1 search+2fresh); known LJ38 SLM exercises one complete outer callback plus a second budget-interrupted attempt, with a1000-request boundary (1000search+1fresh). Both costs equal checkpoint accounting. The latter returns evaluation_failed at the deliberately imposed cap; this is expected containment, not an algorithm success. Neither is random-search evidence. Geometry preflight1488122: shuffled/rotated LJ38 and55 match; known LJ38 SLM does not. Earlier1488078 checked the two positive cases then failed because the old SLM artifact lives in the parent worktree, not this checkout; the absolute source path fixes that input lookup, and the failed log remains.

Independent runner review confirms the paper wrapper changes only direction_sampling. The LJ38 gate responds to a force-qualified energy candidate; geometric positive-control success remains pending independent readout. `completed_outer_attempts` in raw summaries counts records and may include an interrupted final attempt; compare_lj_pilots.py instead reports completed callbacks and charges all remaining costs separately.
