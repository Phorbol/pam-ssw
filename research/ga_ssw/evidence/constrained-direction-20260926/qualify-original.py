...
def main():
    ...
    ledger.dump(out/'protocol.json',dict(case=args.case,config=cfg,direction=direction,
        hookean=constraints.hookean_specs,fixed=constraints.fixed_indices,
        model=model,model_head='omol' if args.case=='c4h6-point' else None,
        seed=25092611,search_cap=cap,fresh_cap=6,wall_seconds=wall,
    ...
    try:
        ...
        for split in (False,True):
            ...
            r=run_constrained_ssw(atoms,surface,steps=1 if split else 2,config=cfg,
                rng=np.random.default_rng(25092611),recovered_direction=direction)
            if split:
                ...
                r=run_constrained_ssw(atoms,surface,steps=1,config=cfg,
                    rng=np.random.default_rng(999),checkpoint=cp,recovered_direction=direction)
            ...
            state=ledger._jsonable(dict(initial=r.initial,current=r.current,best=r.best,
                ...
                direction=r.checkpoint.recovered_direction_state,requests=r.requests))
