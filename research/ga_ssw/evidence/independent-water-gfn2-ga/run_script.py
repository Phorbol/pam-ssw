...
def main():
    output = Path('research/ga_ssw/evidence/independent-water-gfn2-ga')
    ...
    surface = TracedSurface(TBLite(method='GFN2-xTB', verbosity=0), force_consistent=True)
    ...
    ga = PaperGAConfig(quick_steps=1, generations=1, generation_steps=1,
        ...
        quench_fmax=.01, quench_steps=200, proposal_max_batches=2,
    ...
    try:
        ...
        for row in result.archive:
            fresh = ASESurface(TBLite(method='GFN2-xTB', verbosity=0), force_consistent=True)
