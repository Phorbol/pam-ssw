...
def package_imports():
    ...
    from pamssw.standalone.paper_reference import SSWConfig, run_ssw, save_ssw_checkpoint
...
def run_arm(args):
    ...
    try:
        ...
        if result.checkpoint is not None:
            ...
            api["save_ssw_checkpoint"](out / checkpoint_file, result.checkpoint)
        result_payload = {key: value for key, value in vars(result).items()
                          if key != "checkpoint"}
        write_json(out / "search-result.json", {"status": status,
            "search_requests": search_surface.requests, "adapter_calls": None,
            "checkpoint_file": checkpoint_file, "result": result_payload})
    except Exception as exc:
        status = "exception"
...
def run_emt_preflight(args):
    """Run the ordinary worker path on the qualified Native-LS EMT fixture."""
    ...
    with tempfile.TemporaryDirectory(prefix="ls-pool-emt-") as temp:
        ...
        input_path = temp_path / "cu13.extxyz"
        write(input_path, atoms)
        case.update(input=str(input_path), config={"width": .2, "rotation_bias": .5,
            "max_gaussians": 1, "temperature_K": 300., "fmax": .01, "relax_steps": 150,
            "fd_step": 1e-4, "rotation_hvp": 41, "rotation_tol": .02,
            "forward_force": .1, "direction_sampling": "global", "rotation_solver": "ritz",
            ...
            native_ls={"bond_energies": {"(29, 29)": 3.},
                "bond_lengths": {"(29, 29)": 2.8}, "scale": .1},
            recovered_rotation=None, recovered_direction=None,
            native_mc={"energy_tol_eV": .1, "maxtrap": 2})
        plan["cases"] = [case]
        plan["methods"] = [args.mode]
        plan["steps"] = 2
        plan["seed"] = 7
        plan["selector_seed"] = 8
        plan["model"] = {"kind": "emt"}
        plan["pool"] = {"identity_matcher": "ase_permute_v1",
                         "energy_tol": 1e-5, "rmsd_tol": 1e-3}
    ...
    if rc == 0:
        write_json(args.out.resolve() / "preflight.json", {
            ...
            "fixture": "Icosahedron Cu13 with Native-LS/Mc settings from tests/standalone/test_pool_checkpoint_real.py and test_ls_pool_restart.py",
