...
User explicitly requested another attempt. Job1479598 uses unchanged12 prepared cases/core/model, excludes4v100n06, enables faulthandler and has19-minute limit. Prior attempts consumed6 allocated GPU seconds and zero PES; this attempt remains within20-minute total and12024 requests. The prior missing-only derived analysis is preserved as analysis-before-retry.json. No automatic further retries.

Retry result:1479598 CANCELLED by UID0 after1s, no program logs or runs/; Reason/AdminComment/SystemComment empty. CPU1479600 completed readout, all12 cases still missing. No PES executed. Repeated unexplained system cancellation blocks this GPU experiment; no further retry submitted.

Account check: sjtu-caoxiaoming permits rush-1o2gpu in the visible user association. Balance API unavailable without an SAI Identity login. Startup guard checks per-GPU CPU allocation; no account check in the readable taskprolog. On4v100n05, same-account probes1480694(gpus-per-node) and1480698(gres) both COMPLETED0 in1s, reporting GPU1/CPU8. This rules out a persistent account startup block and a reproducible flag-specific failure in these probes; it does not establish the historical cancellation cause or balance. Use the official gpus-per-node spelling for the next bounded original experiment on the tested node, with19min cap and unchanged12024 requests. No scientific settings changed.

Discriminating startup probes on4v100n05:1480712 same19min/account/resources with --export=ALL,PYTHONFAULTHANDLER=1 CANCELLED by0 in2s;1480722 removes only that export option and COMPLETED0 in1s (GPU1/CPU8). This implicates the explicit submission-export path, not a persistent account/wall-limit denial. Underlying platform mechanism remains unknown. Move PYTHONFAULTHANDLER into shell script; no security/guard bypass or account change. Original first segfault is separate.

Recovery confirmed:1480730 COMPLETED0 in44s with unchanged group account. All12 cases completed,202 quench requests+24 fresh=226 total, all24 fresh forces qualified. CPU1480731 performs geometry readout. This supports submission export handling as the reproducible trigger; exact platform internals and account balance remain unverified. No algorithm parameter changed.
