...
OUT=Path('research/ga_ssw/evidence/tio2-vc-structure-identity');OUT.mkdir(exist_ok=False)
...
ss={};report=dict(requests=0,versions={n:importlib.metadata.version(n) for n in ['pymatgen','numpy','spglib']},configuration=dict(scale=False,primitive_cell=False,attempt_supercell=False,allow_subset=False,comparator='SpeciesComparator'),sources={},controls=[],pairs=[],shells={})
...
def compare(a,b,dist,ltol,angle):
 ...
 m=StructureMatcher(stol=dist/ell,ltol=ltol,angle_tol=angle,primitive_cell=False,scale=False,attempt_supercell=False,allow_subset=False,comparator=SpeciesComparator())
