"""Prepared bounded TiO2 fixed-cell holdout; requires explicit --execute."""
import argparse
import hashlib
import json
import os
...
ROOT=Path(__file__).resolve().parents[2]; EVIDENCE=ROOT/'research/ga_ssw/evidence/omat-small-tio2-stress'; MODEL=Path('/home/gengjianrui/.cache/mace/mace-omat-0-small.model'); SEEDS=(11,29); CAP=1200; WALL=300
...
def main(execute=False):
    out=ROOT/'research/ga_ssw/evidence/tio2-fixed-ritz-staged-holdout-20260912'; out.mkdir(parents=False,exist_ok=False)
    shutil.copytree(ROOT/'pamssw',out/'source'/'pamssw',ignore=shutil.ignore_patterns('__pycache__','*.pyc')); shutil.copytree(EVIDENCE,out/'input-evidence'); shutil.copy2(__file__,out/'runner.py')
    ...
    for system in ('rutile','anatase'):
      atoms=read(EVIDENCE/f'{system}.extxyz'); write(out/f'{system}.extxyz',atoms)
      for seed in SEEDS:
