"""26 calls maximum, 240 s CPU E/F/stress preflight of two published 48-atom TiO2 cells."""
import hashlib
...
OUT = Path('research/ga_ssw/evidence/omat-small-tio2-phase87-139-stress')
OUT.mkdir(parents=True, exist_ok=False)
START = time.monotonic()
REPORT = dict(status='running', request_cap=26, requests=0, timeout_seconds=240,
...
def deadline(signum, frame):
    raise TimeoutError('240 second TiO2 preflight limit')
...
try:
    ...
    def evaluate(atoms,label):
        ...
        return result
    for name in ('phase-87','phase-139'):
        path=Path('literature/benchmark-sources/coordinates')/(name+'.extxyz')
        atoms=read(path)
        if len(atoms)!=48 or atoms.get_chemical_formula()!='O32Ti16' or not atoms.pbc.all():
            raise ValueError('expected published 48-atom fully periodic TiO2 cell')
