...
out=Path('research/ga_ssw/evidence/rc-water-dimer-gfn2');out.mkdir(exist_ok=False)
a=molecule('H2O');b=a.copy();b.rotate(180,'z');b.translate([0.,2.9,0.]);a+=b
...
plan=dict(model='tblite GFN2-xTB accuracy .001 neutral singlet; approximate molecular PES',input='two ASE H2O molecules, second rotated180deg around z then translated (0,2.9,0)A; development dimer input, not paper reference',positions=a.positions.tolist(),symbols=a.get_chemical_symbols(),trees=trees,anchor=0,seed=3,steps=1,config=asdict(config),request_cap=400,seconds=30,note='explicit development metric/width same as prior butane probe, not fitted defaults; no parameter tuning; numerical feasibility only')
