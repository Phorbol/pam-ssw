...
import numpy as np
ROOT=Path('/home/gengjianrui/bin/pam-ssw-worktrees/ga-ssw-behavior-parity'); OUT=ROOT/'research/ga_ssw/evidence/xxxii-rc-vc-central-ritz-completion'; SRC=ROOT/'research/ga_ssw/evidence/xxxii-lammps-qualification-table0-ewald12'; TOPO=Path('/home/gengjianrui/bin/pam-ssw-research/ga-ssw-20260909/GA-SSW_examples_run/global_exploration/input-templates/TYPE2-XXXII/mc'); FIX=ROOT/'tests/standalone/fixtures/type2_xxxii.extxyz'; MODEL=ROOT/'research/ga_ssw/evidence/xxxii-stock-charmm-converted'
...
def prepare():
 ...
 cfg={'rotation_length':1.,'torsion_length':1.,'strain_length':5.,'width':.6,'rotation_bias':100.,'max_gaussians':12,'temperature_K':300.,'forward_force':.1,'gradient_tol':.005,'fmax':.01,'stress_tol':.001,'max_step':.2,'relax_steps':4998,'fd_step':1e-4,'rotation_hvp':100,'rotation_tol':.02,'pressure':0.,'lbfgs_memory':400}
 return {'status':'prepared','pes_calls':0,'input':str(FIX),'input_sha256':sha(FIX),'topology':{'rigidbody':str(TOPO/'rigidbody'),'blist':str(TOPO/'blist'),'components':len(top.components)},'model_files':{n:sha(MODEL/n) for n in ('lmp.data','in.simple','manifest.json')},'config':cfg,'fixed_G':.47570069,'pair_table':0,'ewald_accuracy':1e-12,'seed':3,'steps':1,'budget':{'max_EFS':5000,'search_cap':4998,'seconds':120,'fresh_reserved':2},'chart_dimension':chart.dimension,'scope':'experimental converted GAFF backend; not native whole-engine parity or production benchmark'}
...
def main(execute=False):
 ...
 rc_module.generalized_dimer=central_ritz
 ...
 plan['completion_budget_reason']='Separate completion experiment: existing first5Gaussians cost~1370API; 12stages plus fullcelllanding cannot fit1500. Budget5000API/120s chosen as12*(observedmaxstage325)+~1000landing, not efficiency comparison; all1500failurecosts retained.'
 plan['rotation_substitution']='research-only central Ritz; same100API per rotation (central2/HVP incldirectcertificate), not nativeCBD; other config identical to memory400globalbudgetdimer'
 plan['budget_control']='per-quench iteration ceiling equals shared4998searchAPI ceiling; total budget unchanged5000API incl2fresh. Comparememory10/400; no new convergence threshold.'
 ...
 for name in ('xxxii_lammps_calculator.py','run_existing_lammps.py','compare_vc_arms.py','generalized_ritz_probe.py'):
 ...
 class Counted(ASEStressSurface):
  ...
  def evaluate(self,atoms):
   ...
   try:
    if sum(counts.values())>=5000 or counts[self.role]>=(4998 if self.role=='search' else 2):raise RuntimeError('declared request budget exhausted')
