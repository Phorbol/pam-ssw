...
ROOT=Path(__file__).resolve().parents[2];SRC=ROOT/'research/ga_ssw/evidence/xxxii-rc-vc-central-ritz-completion';OUT=SRC/'supercell-representation';OUT.mkdir(exist_ok=False)
...
(OUT/'plan.json').write_text(json.dumps(dict(EFS=4,representations=[[1,1,1],[1,1,2]],input=str(SRC),scope='equivalentperiodicreplication numericaldiagnostic; notlargerscientificsystem; returnE/replicas andmeanforces; recordenergycontinuity withoutclaimwholetrajectoryfixed'),indent=2)+'\n')
rows=[]
for nz in (1,2):
 engine=lammps(cmdargs=['-log',str(OUT/f'engine-z{nz}.log'),'-screen','none'])
 try:
  ...
  if nz>1:engine.command(f'replicate 1 1 {nz}')
  ...
  for p in points:
   primitive=Atoms(**p['atoms']);a=primitive.repeat((1,1,nz));prism=Prism(a.cell.array,pbc=True,reduce_cell=False);xx,yy,zz,xy,xz,yz=prism.get_lammps_prism()
   ...
   rows.append(dict(replicas=nz,label=p['label'],energy=float(engine.get_thermo('pe'))*units.kcal/units.mol/nz,parts=parts,forces=blocks.mean(axis=0).tolist(),stress=stress.tolist(),force_image_max_difference=float(abs(blocks-blocks[:1]).max()),types=types.tolist(),charges=charges.tolist()))
 finally:engine.close()
summary=[]
for nz in (1,2):
 pair=[x for x in rows if x['replicas']==nz];summary.append(dict(replicas=nz,energy_delta=pair[1]['energy']-pair[0]['energy'],parts_delta={k:pair[1]['parts'][k]-pair[0]['parts'][k] for k in pair[0]['parts']}))
(OUT/'result.json').write_text(json.dumps(dict(EFS=4,rows=rows,summary=summary),indent=2)+'\n');print(json.dumps(summary,indent=2))
