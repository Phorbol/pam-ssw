"""Bounded52-EFS fixed-screening derivative diagnostic; no relax/search."""
...
OUT=Path('research/ga_ssw/evidence/xxxii-lammps-qualification-fixedG-three-step-v2');OUT.mkdir(exist_ok=False)
...
plan=dict(expected_EFS=52,max_EFS=60,max_seconds=120,seed=17,h=[1e-4,5e-5,2.5e-5],atomic_direction=atomdir.tolist(),RC_direction=rcdir.tolist(),strain_length=5.,rotation_length=1.,torsion_length=1.,ewald_accuracy=1e-12,rotation=rotation.tolist(),translation='first complete43atom molecule + cell row0',model='algebraically converted original custom GAFF; wholeengine native comparison still pending',versions={p:importlib.metadata.version(p) for p in ('ase','numpy','lammps')},threads={k:os.environ.get(k) for k in ('OMP_NUM_THREADS','OPENBLAS_NUM_THREADS','MKL_NUM_THREADS')},scope='No quench/search; report measured errors, do not infer stable minimum')
plan.update(numerical_control='pair table0/Ewald1e-12 retained; G fixed to first original logged0.47570069 A^-1; third central-difference step2.5e-5 tests truncation; no physical PES parameter changed',reference='docs/research/2026-09-10-xxxii-erfc-floor-audit.md; https://docs.lammps.org/kspace_modify.html',hypothesis='known polynomial force mismatch explains cell residual; RC remaining may be O(h^2) truncation. Test, do not assume.',max_EFS=60,max_seconds=120,authorization='user ongoing bounded CPU algorithm-validation authorization; no GPU/HPC')
import ase,sys,lammps
plan['runtime_versions']={'python':sys.executable,'ase':ase.__version__,'numpy':np.__version__,'lammps':lammps.__version__,'ase_file':ase.__file__}
...
class Table0Diagnostic(XXXIILammpsCalculator):
 ...
 def _initialize(self):
  ...
  self._lmp.command('kspace_style ewald 1e-12')
  self._lmp.command('kspace_modify gewald 0.47570069')
...
def evaluate(b,label,calculator=None):
 if len(calls)>=60 or time.monotonic()-start>=120:raise RuntimeError('approvedqualificationbudget')
