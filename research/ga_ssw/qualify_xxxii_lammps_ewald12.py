...
OUT=Path('research/ga_ssw/evidence/xxxii-lammps-qualification-table0-ewald12');OUT.mkdir(exist_ok=False)
...
plan=dict(expected_EFS=36,max_EFS=40,max_seconds=60,seed=17,h=[1e-4,5e-5],atomic_direction=atomdir.tolist(),RC_direction=rcdir.tolist(),strain_length=5.,rotation_length=1.,torsion_length=1.,ewald_accuracy=1e-12,rotation=rotation.tolist(),translation='first complete43atom molecule + cell row0',model='algebraically converted original custom GAFF; wholeengine native comparison still pending',versions={p:importlib.metadata.version(p) for p in ('ase','numpy','lammps')},threads={k:os.environ.get(k) for k in ('OMP_NUM_THREADS','OPENBLAS_NUM_THREADS','MKL_NUM_THREADS')},scope='No quench/search; report measured errors, do not infer stable minimum')
plan.update(numerical_control='table0 retained; sole changed factor Ewald accuracy1e-12; matched36points, original72EFS failures retained',prior_development_EFS=72,authorization='root approved two precision levels <=80EFS/120s total; each capped40EFS/60s; no physical parameter changes')
