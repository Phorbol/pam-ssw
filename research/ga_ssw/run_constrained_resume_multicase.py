...
def main():
    ...
    env['PAM_CONSTRAINED_RESUME_CHILD'] = '1'
    env['PAM_CONSTRAINED_RESUME_OUTPUT'] = str(out)
...
def _child(out):
    ...
    if not imported.is_relative_to(source_root):
        raise RuntimeError(f'child imported non-snapshot pamssw: {imported}')
    zero_pes_validation_preflight()

    def fixture(element):
        data = json.loads((FIXTURE / f'{element}-seed3-dimer-result.json').read_text())
    ...
    def run_case(element):
        ...
        class Counted(ASESurface):
            def __init__(self, calculator, ledger_path):
            ...
            def evaluate(self, candidate):
                if self.requests >= cap:
                    self.record(dict(event='request_cap_denial', charged=False,
                                     requests=self.requests, shared_paid=shared.paid))
        ...
        def execute(surface, steps, rng, checkpoint=None, checkpoint_path=None):
            surface.started = time.monotonic()
        ...
        folder = out / element
        folder.mkdir()
        cont_surface = Counted(EMT(), folder / 'continuous-ledger.json')
        first_surface = Counted(EMT(), folder / 'first-ledger.json')
        resume_surface = Counted(EMT(), folder / 'resumed-ledger.json')
        ...
        row = dict(element=element,
                   ...
                   shared_paid=shared.paid, resumed_total_requests=result_requests(resumed),
    ...
    for element in ('Cu', 'Al'):
        fixture_data = json.loads((FIXTURE / f'{element}-seed3-dimer-result.json').read_text())
    ...
    dump(out / 'plan.json', dict(cases=['Cu', 'Al'], seed=3, solver='dimer', steps=2,
        split=[1, 1], request_cap=12000, wall_seconds=60,
        configs=configs, source_fixture=str(FIXTURE),
        purpose='constrained checkpoint and paid-request replay; no solver-quality claim'))
    rows = []
    for element in ('Cu', 'Al'):
        try:
...
if __name__ == '__main__':
    if os.environ.get('PAM_CONSTRAINED_RESUME_CHILD') == '1':
        _child(Path(os.environ['PAM_CONSTRAINED_RESUME_OUTPUT']))
    else:
        main()
