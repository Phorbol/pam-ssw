"""Bounded EMT qualification of competing Cu55/Al55 cluster motifs."""
...
def main():
    ...
    for element in ('Cu','Al'):
        cases[f'{element}55_ico']=Icosahedron(element,3)
        cases[f'{element}55_octa']=Octahedron(element,5,cutoff=2)
    plan={'status':'bounded_motif_qualification_not_search','cases':list(cases),'atoms_per_case':55,
          ...
          'constructors':{'ico':'Icosahedron(symbol,3)','octa':'Octahedron(symbol,5,cutoff=2)'},
          'source_commit':git,
    ...
    # Invariant structural comparison after independent relaxations.
    comparisons=[]
    from ase.io import read
    for element in ('Cu','Al'):
        aa=read(out/f'{element}55_ico/relaxed.extxyz'); bb=read(out/f'{element}55_octa/relaxed.extxyz')
        fa,fb=pair_fingerprint(aa),pair_fingerprint(bb); delta=np.abs(fa-fb)
        comparisons.append({'composition':element,'pair_fingerprint_max_abs_difference_A':float(delta.max()),'pair_fingerprint_rms_A':float(np.sqrt(np.mean(delta**2))),'same_atom_count':len(aa)==len(bb),'identity_note':'permutation/rigid-motion invariant sorted pair distances; non-injective diagnostic'})
    (out/'motif-comparison.json').write_text(json.dumps(comparisons,indent=2)+'\n')
