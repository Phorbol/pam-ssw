"""Bounded CuO64 PQC versus joint comparison runner.
...
``input.json`` (the frozen, qualified 64-atom ASE JSON) and ``plan.json``.
...
def main(argv=None):
    ...
    if len(atoms) != 64 or not atoms.pbc.all():
        raise ValueError("input.json must be the qualified fully periodic 64-atom cell")
    ...
    script_copy = out / "run_cuo_vc_comparison.py"
    ...
    for seed in (7, 101):
        for arm in ("pqc", "joint"):
            ...
            try:
                report = run_material_arm(
                    atoms.copy(), surface, arm=arm, atomic_config=atomic,
                    block_config=block, joint_config=joint, steps=2, seed=seed)
            except Exception as error:
