"""Bounded Fe7C3-80 joint VC fixed-forward-force versus analytic87-degree height comparison.
...
def run_angle_arm(atoms, surface, *, arm, joint, seed):
    settings=None
    ...
    return dict(arm=arm,seed=seed,steps_requested=2,ls=settings,height_policy="MinimalAngleHeightPolicy",joint_config=joint,
...
def main(argv=None):
    ...
    script_copy = out / "run_fe7c3_angle_comparison.py"
    ...
    for seed in (7, 101):
        for arm in ("minimal_angle",):
            ...
            try:
                report = run_angle_arm(atoms.copy(), surface, arm=arm, joint=joint, seed=seed)
            except Exception as error:
