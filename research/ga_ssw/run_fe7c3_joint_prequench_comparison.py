"""Bounded Fe7C3-80 joint soft-prequench LS comparison against frozen fixed-cell preparation controls.
...
def run_ls_arm(atoms, surface, *, arm, joint, seed):
    ...
    result=run_vc_ssw(atoms,surface,steps=2,config=joint,rng=np.random.default_rng(seed),ls=settings,ls_prequench="joint")
    landings=[]
    if result.minima:
        ev=result.minima[0]
    ...
    return dict(arm=arm,seed=seed,steps_requested=2,ls=settings,ls_prequench="joint",joint_config=joint,
...
def main(argv=None):
    ...
    script_copy = out / "run_fe7c3_joint_prequench_comparison.py"
