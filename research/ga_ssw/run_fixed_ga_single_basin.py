"""Bounded single-basin TYPE0 GA parent-establishment study; explicit --execute required."""
...
def main():
    ...
    for seed in (11,29):
        ...
        class Bounded(ASESurface):
            denied=0
            def evaluate(self,a):
                if self.requests>=12000 or time.monotonic()-started>=120: self.denied+=1; raise RuntimeError('bounded search cap or wall limit')
                try: e,f=super().evaluate(a)
                except Exception as error:
                    with ledger.open('a') as h:h.write(json.dumps(serial(dict(request=self.requests,error=repr(error),atoms=a)))+'\n')
                    raise
