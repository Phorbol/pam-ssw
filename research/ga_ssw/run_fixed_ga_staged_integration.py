"""Public staged Ritz direction integration with GA and both LS variants on five G2 C4H6 isomers.
...
def main():
    ...
    molecules = {name: g2[name].copy() for name in g2.names
                 if g2[name].get_chemical_formula() == "C4H6"}
    references = [molecules[n] for n in ("butadiene", "cyclobutene", "2-butyne")]
    bond_limits = {k: 0.7 * v for k, v in HC_BOND_LENGTHS.items()}
    cfg = PaperGAConfig(quick_steps=1, generations=1, generation_steps=1,
        fine_steps=1, offspring_steps=1, ga_candidates=4, regions=1, fine_regions=1,
        quench_fmax=.01, quench_steps=400, proposal_max_batches=4,
    ...
    ssw = SSWConfig(width=.1, rotation_bias=None, pre_rotation_hvp=5, max_gaussians=3,
    ...
    dump(out/'plan.json', dict(systems=list(molecules), seeds=[11, 29], arms=list(arms),
        ...
        references='Three actual G2 isomers: butadiene, cyclobutene, 2-butyne',
        quota_source='TYPE0 multi-element G//4 crossovers: G=4 is first nonzero quota. Up to four batches to reach four candidates; no mutation at this G. All five G2 C4H6 isomers supplied to avoid a two-parent-only population; no invented parents or outcome-selected subset.',
        ...
        scope='Shared staged Ritz integration validation; same frozen offspring refinement protocol, only direction changed to staged Ritz. Development data, not independent efficiency test. Shared ordinary temperature150K explicitly retained to isolate lifecycle; not native5x temperature reproduction. No LS advantage or GA efficiency claim'))
                ...
                try:
                    ...
                    for item in sorted(result.archive, key=lambda x:x['energy'])[:10]:
                        try:
                            ...
                            e, f = fresh.evaluate(item['atoms'])
                            check = dict(id=item['id'], energy=e, energy_error=e-item['energy'],
                            ...
                            check = dict(id=item['id'], error=repr(error))
                        checks.append(check)
                        row.update(fresh_requests=fresh.requests, fresh_checks=checks,
                                   unverified_archive=max(0,len(result.archive)-len(checks)))
