"""Prepare the bounded C4H6 SSW/paper-LS/native-LS lifecycle comparison.

The script is preparation-only unless ``--execute`` is supplied.  It runs the
three public fixed-cell SSW entry points from one frozen source snapshot.
"""
...
def main():
    ...
    ap.add_argument("--source-root", type=Path, default=Path(
        "research/ga_ssw/evidence/fixed-ga-single-basin-20260912/source"))
    args = ap.parse_args()
    ...
    plan = dict(
        ...
        seeds=[11, 29], variants=list(arms), steps=400, max_evaluations=40000,
        wall_seconds=120, config=asdict(config), ls_settings=arms,
        ...
        scope="bounded developmental connection-coverage comparison; 400 is an outer-step limit, not 400-minima paper reproduction; no global-minimum or LS-advantage claim",
        ...
        for seed in (11, 29):
            ...
            class Counted(ASESurface):
                ...
                def evaluate(self, atoms):
                    if self.requests >= 40000 or time.monotonic() - started >= 120:
                        self.denied += 1
                        self.boundary = "request_cap" if self.requests >= 40000 else "wall_cap"
            ...
            fresh = None; checks = []
            try:
                kwargs = dict(atoms=initial.copy(), surface=surface, steps=400,
