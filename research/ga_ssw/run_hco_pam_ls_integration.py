...
def main():
    ...
    plan=dict(purpose='Public staged Ritz plus existing PAM height_width Gaussian with paper/native LS; real HCO composition integration, not effect benchmark',
              seeds=[11,29],steps=2,per_arm_total_EF=3000,fresh_reserve=3,per_arm_seconds=90,
              variants={name:['ls_paper','ls_native'] for name in cases},
              parameter_source='Existing fixed-cell diagnostic width .1 stagedRitz pre5 total100 plus PAM height_width relax400; 3 Gaussians/attempt; bias_fmax .1, outer .01. Explicit recovered HCO lookup tables; paper target .02 eV/atom and native target20 meV/atom are diagnostic defaults, not GFN2-fitted parameters.',
    ...
    config=SSWConfig(width=.1,rotation_bias=None,pre_rotation_hvp=5,max_gaussians=3,temperature_K=150.,
        fmax=.01,bias_fmax=.1,relax_steps=400,fd_step=1e-4,rotation_hvp=100,
        rotation_tol=.02,rotation_solver='ritz',cluster_frame='direction_only',
    ...
    dump(out/'parameters.json',dict(config=asdict(config),paper_ls=asdict(paper_ls),native_ls=asdict(native_ls),gaussian_policy=PAMCurvatureGaussian().parameters()))
    ...
    for name,a in cases.items():
      ...
      for variant in plan['variants'][name]:
       for seed in plan['seeds']:
        ...
        try:
            kw=dict(steps=2,config=cfg,rng=np.random.default_rng(seed),gaussian_policy=PAMCurvatureGaussian())
