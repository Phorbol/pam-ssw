...
def main():
    ...
    parser.add_argument('--fd-step', type=float, default=.001)
    parser.add_argument('--proposal-batches', type=int, default=2)
    parser.add_argument('--pair-attempts', type=int, default=100)
    ...
    surface = TracedSurface(TBLite(method='GFN2-xTB', verbosity=0, accuracy=args.accuracy), force_consistent=False)
    ...
    ssw = SSWConfig(width=.2, rotation_bias=100., max_gaussians=2,
        temperature_K=300., fmax=.01, relax_steps=args.relax_steps, fd_step=args.fd_step,
    ...
    ga = PaperGAConfig(quick_steps=1, generations=1, generation_steps=1,
        ...
        quench_fmax=.01, quench_steps=args.quench_steps, proposal_max_batches=args.proposal_batches,
        proposal_max_cut_attempts=100, proposal_max_pair_attempts=args.pair_attempts,
    ...
    config = dict(backend='tblite 0.7.0 GFN2-xTB', source_paths=list(map(str, paths)),
        ...
        electronic_accuracy=args.accuracy,
    ...
    try:
        ...
        for row in result.archive:
            fresh = ASESurface(TBLite(method='GFN2-xTB', verbosity=0, accuracy=args.accuracy), force_consistent=False)
