...
def main():
    ...
    for name,atoms in systems:
      for label,prefix in [('preserve-first',[0.,.2,0.]),('replace-first',[.9,.7,0.]),('neighbor-first',[0.,.2,.9])]:
        try:
          row=run(atoms.positions,(1,2),prefix,atoms.numbers,np.zeros(len(atoms)))
        ...
        cases.append(row)
    report=dict(scope='expanded fixed inputs; bounded get_atompair_v2; no main/PES',case_count=len(cases),cases=cases)
    Path('research/ga_ssw/evidence/native-getpair-expanded-20260917.json').write_text(json.dumps(report,indent=2)+'\n')
