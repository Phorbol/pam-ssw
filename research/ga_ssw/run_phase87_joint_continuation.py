...
OUT=ROOT/'research/ga_ssw/evidence/tio2-phase87-vc-pqc-single-step/joint-continuation-v1'
...
q0=np.asarray(rec['climb'][-1]['q'],float)
assert q0.shape==(150,)
last=atom(rec['last_work']); q_unpacked=chart.unpack(q0)
assert np.array_equal(last.numbers,q_unpacked.numbers)
assert np.max(np.abs(last.positions-q_unpacked.positions))==0.
assert np.max(np.abs(last.cell.array-q_unpacked.cell.array))==0.
...
plan=json.loads((OUT/'plan.json').read_text()); plan.update({'script_sha256':hashlib.sha256(Path(__file__).read_bytes()).hexdigest(),'model_sha256':hashlib.sha256(MODEL.read_bytes()).hexdigest(),'zero_pes_validation':{'q_last_work_position_diff':0.0,'q_last_work_cell_diff':0.0,'terms':8,'anchor_reconstructed_from_seed3':True}}); dump('plan.json',plan)
...
# Complete the saved stage's missing physical evaluation. This is the only evaluation needed before stage 8.
ev=physical(q); rows.append(dict(role='resume_stage7_true_eval',index=7,objective=ev.objective,delta_from_current=ev.objective-current_obj,requests=surface.requests))
if ev.objective < current_obj:
