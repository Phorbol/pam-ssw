"""Research-only two-stage plane-dimer rotation diagnostic.
...
def two_stage_dimer_direction(atoms, anchor, *, fd_step, max_hvp,
    ...
    ``dimer`` (the default) or ``ritz``.  ``max_hvp`` retains the old
    ...
    if main_solver not in ('dimer', 'ritz'):
        raise ValueError("main_solver must be 'dimer' or 'ritz'")
    main_min_hvp = 1 if main_solver == 'dimer' else 2
    ...
    bias = max(ce, 0.)
    solver = paper_dimer_direction if main_solver == 'dimer' else paper_biased_direction
