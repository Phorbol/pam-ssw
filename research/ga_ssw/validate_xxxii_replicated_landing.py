...
OUT=SRC/'cross-representation-validation'
...
initial=read(SRC/'minimum-0.extxyz')
rows=[];start=time.monotonic()
(OUT/'plan.json').write_text(json.dumps(dict(EFS=4,representations=[[1,1,3],[2,2,2]],points=['minimum-0','minimum-1'],scope='equivalent representation and independent physical tolerance check, not Hessian stability or global search efficiency'),indent=2)+'\n')
for index in (0,1):
    a=read(SRC/f'minimum-{index}.extxyz')
