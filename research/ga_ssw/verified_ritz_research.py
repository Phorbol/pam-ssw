"""Experimental verified Ritz soft-mode solver, independent of PAM policies.
...
def reference_soft_mode(atoms, initial_direction, *, fd_step, max_hvp,
    ...
    for _ in range(max_basis):
        # A direct certification consumes a HVP outside the basis-growth
        # iteration.  Stop growing before the last available slot so the
        # final current candidate can still receive exactly one certificate.
        ...
        if np.linalg.norm(surrogate_residual)<=residual_tol:
            # A Ritz residual is only a subspace certificate.  Spend one
            # direct HVP on the candidate; if that certificate fails, keep
            # growing the existing Krylov basis while budget remains.
