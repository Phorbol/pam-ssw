#!/usr/bin/env python3
"""Render deterministic trace curves for the reviewed inverse-scale ledger."""
...
ARM_LABELS = {
    "fixed-scale-history0": "Fixed scale, history 0",
    "adaptive-scale-history0": "Adaptive scale, history 0",
    "adaptive-scale-history10": "Adaptive scale, history 10",
}
ARM_LINESTYLES = {
...
SVG_METADATA = {
    "Creator": "pamssw deterministic inverse-scale plotter",
...
@contextmanager
def _scale_decomposition_figure(plt: Any) -> Iterator[tuple[Any, Any]]:
...
def render_plot(rows: Sequence[Mapping[str, Any]], output_path: Path) -> None:
    os.environ.setdefault(
        "MPLCONFIGDIR",
        str(Path(tempfile.gettempdir()) / "pamssw-scale-decomposition-matplotlib"),
    )
    import matplotlib
    ...
    with (
        plt.rc_context(
            {
                ...
                "path.simplify": False,
                "svg.fonttype": "none",
                "svg.hashsalt": "pamssw-safe-scale-decomposition-v1",
            }
        ),
        _scale_decomposition_figure(plt) as (figure, axes),
    ):
        figure.set_gid("safe-scale-decomposition-curves")
        ...
        figure.suptitle(
            "Safe L-BFGS inverse-scale decomposition traces",
            fontsize=14,
            y=0.985,
        )
