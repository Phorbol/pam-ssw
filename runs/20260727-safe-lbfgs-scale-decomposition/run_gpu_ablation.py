#!/usr/bin/env python3
"""Fail-closed, one-shot safe L-BFGS inverse-scale GPU decomposition.

This runner intentionally evaluates only three fixed arms over the 16 frozen
...
EXPECTED_PAMSSW_BUNDLE_SHA256 = "96761a45dfe7c8af459ba8112adb79efee4d53ce073a74f7d87fea09c35c9d2a"
...
EXPECTED_SAFE_KERNEL_DESCRIPTOR = {
    ...
    "scale_policies": {
        ...
        "latest-history-pair-gamma-plus-two-loop": (
            "gamma=(s.T@y)/(y.T@y) from newest history pair plus two-loop corrections"
        ),
    },
    "kernel_constants": {
...
EXPECTED_SAFE_KERNEL_DESCRIPTOR_SHA256 = "ba0c261cd588ce72e8175332385389e69cf8840c00399318723af7e0358e7f24"
...
ARMS = (
    Arm(
        arm_id="fixed-scale-history0",
        kernel="safe-lbfgs-total",
        history_limit=0,
        adaptive_scale_without_history=False,
        scale_policy="fixed-1-over-70",
    ),
    Arm(
        arm_id="adaptive-scale-history0",
        kernel="safe-lbfgs-total",
        history_limit=0,
        adaptive_scale_without_history=True,
        scale_policy="latest-accepted-secant-gamma",
    ),
    Arm(
        ...
        history_limit=10,
        adaptive_scale_without_history=False,
        scale_policy="latest-history-pair-gamma-plus-two-loop",
    ),
)
...
def _validate_full_ledger_rows(rows: Sequence[Mapping[str, Any]]) -> None:
    ...
    try:
        ...
        raise ValueError("48-row ledger rows require system, task_id, and arm_id") from error
    if len(rows) != 48 or len(observed) != 48 or observed != expected:
        raise ValueError("48-row ledger does not contain exactly every fixed task and arm")
...
def publish_ledger_atomically(
    ...
    *,
    ...
    """Publish only a complete 48-row ledger with one directory rename."""
...
def _row_payload(
    ...
    return {
        ...
        "adaptive_scale_without_history": arm.adaptive_scale_without_history,
...
def run(
    *,
    ...
    summary = {
        "schema_version": 1,
        "claim_ceiling": (
            "Fixed-task scalar-scale versus two-loop-correction CUDA replay evidence "
            "only; no endpoint-equivalence, one-versus-multi-secant, SSW-performance, "
            "statistical, or default-change claim."
        ),
