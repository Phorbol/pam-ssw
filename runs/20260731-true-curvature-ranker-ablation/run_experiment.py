#!/usr/bin/env python3
"""Run one prospective static-score versus true-curvature ranker cohort."""
...
protocol = _load_module(
    PROTOCOL_PATH,
    "_true_curvature_ranker_protocol_runner",
)
counterfactual = _load_module(
    COUNTERFACTUAL_RUNNER_PATH,
    "_true_curvature_counterfactual_runtime",
)
...
def _run_case(
    *,
    ...
    landing = walker.relax_true_minimum(
        escape_state,
        trajectory_name=f"{arm}-landing",
    )
    quench_wall_time = float(perf_counter() - quench_started)
    direction_rows = _read_direction_rows(diagnostics_path)
    if not direction_rows:
        raise RuntimeError("ranker case contains no direction selection")
    if any(
        row.get("direction_ranking_mode") != arm
        for row in direction_rows
    ):
        raise RuntimeError("direction trace does not match the requested ranker")
    if arm == "true_curvature" and any(
        abs(
            float(row["direction_ranking_score"])
            + float(row["true_curvature"])
        )
        > 1.0e-8
        for row in direction_rows
    ):
        raise RuntimeError("true-curvature trace did not rank by -curvature")
    purpose_counts = walker.calculator.snapshot().as_dict()
    if purpose_counts["unattributed"] != 0:
        raise RuntimeError("ranker case contains unattributed evaluations")
    if sum(purpose_counts.values()) != walker.calculator.force_evaluations:
        raise RuntimeError("ranker case purpose ledger does not close")
    certificate = bool(
    ...
    row = {
        ...
        "selected_direction_kinds": [
            row["selected_kind"] for row in direction_rows
        ],
...
def run(
    *,
    ...
    audit = _load_module(
        ...
        f"_true_curvature_fixed_state_audit_{repeat_id}",
    )
    config_gate = _load_module(
        counterfactual.CONFIG_GATE_PATH,
        f"_true_curvature_config_gate_{repeat_id}",
    )
        ...
        for state_id in protocol.STATE_IDS:
            ...
            for seed in protocol.SEEDS:
                ...
                for arm in protocol.ARMS:
                    print(
                        f"[ranker] repeat={repeat_id} {system} "
                        f"{state_id} seed={seed} arm={arm}",
                        flush=True,
                    )
                    row = _run_case(
                        state=state,
                    ...
                    if (
                        ...
                        raise RuntimeError(
                            "ranker cohort exceeded its 30000 FE stop"
                        )
                    _write_json(
